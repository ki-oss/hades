...
async def test_logging_hades(caplog, capsys):
    ...
    hades = Hades()
    setup_date_logging(
        hades, fmt="%(levelname)-8s %(name)s [%(world_date)s] %(message)s"
    )  # drop line number to make less sensitive to code changes
    hades.register_process(YearStartScheduler(start_year=2023, look_ahead_years=2))
    await hades.run()
    assert (
        caplog.text
        == """INFO     hades.core.hades [0001-01-01] registered process: YearStartScheduler, instance: 332231294394531790607923355838092946842
INFO     hades.core.hades [0001-01-01] registered process: HadesInternalProcess, instance: 7836064115094481643618470001379502846
DEBUG    hades.core.hades [0001-01-01] adding SimulationStarted from process: HadesInternalProcess, instance: 7836064115094481643618470001379502846 (caused by None) to queue
...
DEBUG    hades.core.hades [0001-01-01] adding YearStarted from process: YearStartScheduler, instance: 332231294394531790607923355838092946842 (caused by None) to queue
DEBUG    hades.core.hades [0001-01-01] adding YearStarted from process: YearStartScheduler, instance: 332231294394531790607923355838092946842 (caused by None) to queue
