...
@pytest.mark.parametrize("dt, expected_step", [
...
@pytest.mark.parametrize("step, expected_datetime", [
    (300, EPOCH + timedelta(days=300))
])
def test_step_to_datetime(step, expected_datetime):
    assert step_to_datetime(step) == expected_datetime

@pytest.mark.parametrize("step, expected_date", [
    (300, (EPOCH + timedelta(days=300)).date())
])
def test_step_to_date(step, expected_date):
    assert step_to_date(step) == expected_date

@pytest.mark.parametrize("dt, expected_quarter", [
    (datetime(2023, 1, 1), 1),
    (datetime(2023, 4, 1), 2),
    (datetime(2023, 7, 1), 3),
...
@pytest.mark.parametrize("dt, expected_days", [
